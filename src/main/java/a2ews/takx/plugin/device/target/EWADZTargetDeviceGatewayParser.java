 ...
 */
public class EWADZTargetDeviceGatewayParser extends AGatewayParser
{
    /**
     * Parser helper.
     */
     ...
     */
    protected EWADZTargetDeviceGatewayParser(CommServiceType sourceType, String sourceName, IGatewayParserDelegate delegate)
    {
    ...
    public void parseBytes(byte[] bytes, int length, String format)
    {
        for (int i = 0; i < length; i++)
        {
            ...
            if (radarInfo != null)
            {
                if (radarInfo.radarMessageType == RadarParser.RadarMessageType.TARGET)
                {
                    // The message structure must be defined in the data structure xml
                    IRaptorDataStructure dataStructure = factory.getDataStructure(EWADZTargetDeviceConstants.TARGET_MESSAGE);
                    IPrePersistRaptorDataMessage rdm = dataStructure.createRaptorDataMessage(radarInfo.targetId);
